...
const { detectLanguage }  = require('../parsing/language.detector');
const { getParser }       = require('../parsing/parser.registry');
const { JavaScriptParser } = require('../parsing/languages/javascript.parser');
const { TypeScriptParser } = require('../parsing/languages/typescript.parser');
const { PythonParser } = require('../parsing/languages/python.parser');
const { JavaParser } = require('../parsing/languages/java.parser');
const { CppParser } = require('../parsing/languages/cpp.parser');
const { KotlinParser } = require('../parsing/languages/kotlin.parser');
const { createFileAnalysis } = require('../parsing/symbols');
 ...
 * }
 */
async function analyzeRepository(rootDir, previousAnalysis = null, onProgress = null, options = {}) {
  ...
  let sourceFiles;
  try {
    sourceFiles = scanSourceFiles(rootDir, options.ignorePatterns || []);
  } catch (err) {
    result.status = 'error';
 ...
 */
function scanSourceFiles(dir, customIgnores = []) {
  const results = [];
  _scanDir(dir, results, customIgnores);
  return results;
}

function _scanDir(dir, results, customIgnores) {
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (customIgnores.length > 0 && customIgnores.some(pattern => entry.name.includes(pattern))) continue;

    const absPath = path.join(dir, entry.name);

    if (entry.isDirectory()) {
      _scanDir(absPath, results, customIgnores);
