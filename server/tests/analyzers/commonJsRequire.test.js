...
describe('CommonJS require — basic forms', () => {
  test('const x = require("./module") produces an import symbol', () => {
    const imports = getImports(`const db = require('./db');`);
    expect(imports).toHaveLength(1);
    expect(imports[0].source).toBe('./db');
  });

  test('var x = require("./module") produces an import symbol', () => {
    const imports = getImports(`var db = require('./db');`);
    expect(imports).toHaveLength(1);
    expect(imports[0].source).toBe('./db');
  });

  test('let x = require("./module") produces an import symbol', () => {
...
describe('CommonJS require — cjs-default specifier', () => {
  ...
  test('relative require produces cjs-default specifier', () => {
    const imports = getImports(`const User = require('./models/User');`);
...
describe('CommonJS require — location', () => {
  test('location is set on require import', () => {
    const imports = getImports(`const x = require('./foo');`);
...
describe('CommonJS require — integration with dependency graph', () => {
  test('require() import becomes a "requires" edge in the graph', () => {
    const {
  ...
  test('ES import becomes an "imports" edge in the graph', () => {
    const {
      buildDependencyGraph,
    } = require('../../src/analyzers/dependencyGraph');
