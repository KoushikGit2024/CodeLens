...
function graphToFlow(graph, selectedId, showExternalPackages) {
  let nodesToRender = graph.nodes;
  let edgesToRender = graph.edges;

  if (!showExternalPackages) {
    nodesToRender = nodesToRender.filter(n => n.type === 'file');
    const nodeIds = new Set(nodesToRender.map(n => n.id));
    edgesToRender = edgesToRender.filter(e => nodeIds.has(e.source) && nodeIds.has(e.target));
  }
...
export default function DependencyGraphPage() {
  ...
  useEffect(() => {
    if (!graph) return;
    const { rfNodes, rfEdges } = graphToFlow(graph, selected, showExternalPackages);
    setNodes(rfNodes);
    setEdges(rfEdges);
  }, [graph, selected, showExternalPackages]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      panels={[
        {
          ...
          collapsible: true,
          content: (
            <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-4 custom-scrollbar bg-panel h-full">
              {stats && (
                <section>
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
                    <span className="text-xs font-medium text-white">Show External</span>
                    <button
                      onClick={() => setShowExternalPackages(p => !p)}
                      className={`w-8 h-4 rounded-full transition-colors duration-200 ease-in-out ${showExternalPackages ? 'bg-accent' : 'bg-surface border border-border'}`}
                    >
                      <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform duration-200 ease-in-out ${showExternalPackages ? 'translate-x-4 border-accent' : 'translate-x-0 border-transparent'} border`} />
                    </button>
                  </div>
                </section>
              )}
...
function FileDetailPanel({ info, repoId }) {
  const explorerUrl = `/explore/${repoId}/source?path=${encodeURIComponent(info.filePath)}`;
