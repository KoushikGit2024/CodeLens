  ...
  useEffect(() => {
    ...
    async function renderDiagram() {
      if (data?.mermaid && mermaidRef.current) {
        try {
          const { svg } = await mermaid.render('architecture-graph', data.mermaid);
          if (isMounted) {
    ...
    <ResizableLayout
      panels={[
        ...
        },
        {
          ...
          collapsible: false,
          content: (
            <main className="flex-1 overflow-auto bg-[#0d1117] relative flex justify-center p-8 custom-scrollbar h-full w-full">
